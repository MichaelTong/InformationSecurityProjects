#include "StdAfx.h"
#include ".\tcpnullscan.h"
#include "DataAndConst.h"
bool bNullSending= true;
DWORD WINAPI TcpRstReceiverNull(LPVOID pParament)
{
	ThreadSyn cSynEntry;
			...
			if(nRes == 1)
			{
				...
				if(dSourcePort>=pThreadParament->dwOriginalPort&&dSourcePort<=pThreadParament->dwLastPort)
				{
					if(dDestPort==12345)
					{
						...
						pMainWindow->PostMessage(WM_NULL_SCAN,dwStateAndPort,dwIp);
					}
				}
			}
		}
...
DWORD WINAPI TcpNullScan(LPVOID pPar)
{
	ThreadSyn cSynEntry;
	SetBoolTrue cSBT(&bNullSending);
	DWORD dwThreadId;
	CreateThread(NULL,0,TcpRstReceiverNull,pPar,0,&dwThreadId);
