...
bool bAckSending= true;
DWORD WINAPI TcpRstReceiverAck(LPVOID pParament)
{
	ThreadSyn cSynEntry;
			...
			if(nRes == 1)
			{
				...
				if(dSourcePort>=pThreadParament->dwOriginalPort&&dSourcePort<=pThreadParament->dwLastPort)
				{
					if(dDestPort==12345)
					{
						...
						pMainWindow->PostMessage(WM_ACK_SCAN,dwStateAndPort,dwIp);
					}
				}
			}
		}
...
DWORD WINAPI TcpAckScan(LPVOID pPar)
{
	ThreadSyn cSynEntry;
	SetBoolTrue cSBT(&bAckSending);
	DWORD dwThreadId;
	CreateThread(NULL,0,TcpRstReceiverAck,pPar,0,&dwThreadId);
	...
	thePacket.theTcpHead.ucFlag=0x10;
